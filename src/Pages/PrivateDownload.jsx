...
const PublicMedia = () => {
	...
	const [metadataUrl, setMetadataUrl] =
		useState('');
	const [pageData, setPageData] = useState({});
	const [showInputs, setShowInputs] =
		useState(false);
	...
	const generateMetaUrl = (data) => {
		const url = `https://www.instagram.com/graphql/query?query_hash=2b0673e0dc4580674a88d426fe00ea90&variables={%22shortcode%22:%22${data}%22}`;
		setMetadataUrl(url);
		setShowInputs(true);
	};
	...
	const handleSourceSubmit = (data) => {
		...
		if (IsJson(data)) {
			...
			if (parsedData.data.shortcode_media) {
				const mediaData =
					parsedData.data.shortcode_media;
				setMediaInfo({
					profilePic: `https://imageproxy.pimg.tw/zoomcrop?url=${encodeUrl(mediaData.owner.profile_pic_url)}&width=400&height=400`,
					date: mediaData.taken_at_timestamp,
					thumbnail: `https://imageproxy.pimg.tw/zoomcrop?url=${encodeUrl(mediaData.display_url)}&width=1080&height=1920`,
					video_url: mediaData.video_url,
					shortcode: mediaData.shortcode,
					views: mediaData.video_view_count,
					plays: mediaData.video_play_count || 'N/A',
					likes:
						mediaData.edge_media_preview_like.count,
					comments:
						mediaData.edge_media_to_comment.count,
					media_type: mediaData.__typename,
					full_name: mediaData.owner.full_name,
					username: mediaData.owner.username,
					caption:
						mediaData.edge_media_to_caption.edges[0]
							.node.text,
					is_verified: mediaData.owner.is_verified,
					tagged_users:
						mediaData.edge_media_to_tagged_user.edges,
				});
			...
			<div className={styles.mainHome}>
				...
				<div className={styles.upperWrapper}>
					<img
						...
						alt=''
					/>
					<h2>View Private Post Anonymously</h2>
					<p>
						Download Private Stories, Post / IGTV /
						Media in one click using IGLoaded Private
						Downloader!
					</p>
						...
						<>
							<div className={styles.sectionInput}>
								...
								<p className={styles.info}>
									Copy this URL and open in a new window
								</p>
							</div>
							<div className={styles.sectionInput}>
								<div className={styles.inputWrapper}>
									<input
										type='text'
										placeholder='Paste all the page source data here'
										data-tooltip-id='textbox'
										onChange={(e) => {
								...
								<p className={styles.info}>
									Paste all the data from that page in the
									input above
								</p>
							</div>
						</>
					...
					<div className={styles.contentDiv}>
						...
						<div className={styles.postInfo}>
							...
							<div className={styles.profileInfo}>
								...
								{MediaInfo.tagged_users.length > 0 && (
									<>
										<p className={styles.taggedInfoText}>
											Tagged Users
										</p>
										<div className={styles.taggedInfo}>
											{MediaInfo.tagged_users.map(
												(tagged) => {
													return (
														<div
															className={styles.taggedData}
															key={tagged.node.user.id}
															onClick={() => {
																window.open(
																	`https://www.instagram.com/${tagged.node.user.username}`
																);
															}}
														>
					...
					<div className={styles.guideWrapper}>
						<div className={styles.guideItem}>
							...
							<div className={styles.itemRight}>
								<h2>Step 1</h2>
								<p>
									Paste the Instagram URL of the post,
									reel, IGTV, or story you want to view
									anonymously.
								</p>
							</div>
						</div>
						<div className={styles.guideItem}>
							...
							<div className={styles.itemRight}>
								<h2>Step 2</h2>
								<p>
									Copy the URL generated and paste it in
									your browser. Copy the Data you see on
									the page and paste it in the input box.
									(This Requires Instagram Login and you
									should've followed the user to view the
									media)
								</p>
							</div>
						</div>
						<div className={styles.guideItem}>
							...
							<div className={styles.itemRight}>
								<h2>Step 3</h2>
								<p>
									Click on the download button and enjoy
									the media anonymously!
								</p>
							</div>
						</div>
					</div>
